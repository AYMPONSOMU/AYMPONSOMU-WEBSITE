/* AYMP Social Interaction Layer
   ...
   Works across existing cards plus Daily Discovery and Global Trending.
   Comments/likes are local until a shared backend + login system is connected.
*/
(function(){
  'use strict';
  function init(){
    ...
    cards.forEach(function(card,index){
      ...
      var key=card.getAttribute('data-social-id') || ('card-'+index+'-'+location.pathname+'-'+(card.textContent||'').slice(0,30));
      var wrap=document.createElement('div'); wrap.className='aymp-social-bar';
      wrap.setAttribute('data-social-id',key);
      ...
      wrap.addEventListener('click',function(e){
        ...
        if(action==='comment'){
          ...
          if(text){
            localStorage.setItem('aymp-comment-'+key,text);
            alert('Thank you for your comment!');
          }
        }
        if(action==='repost'){
          if(navigator.share){ navigator.share({title:document.title,text:'Discover this AYMP content',url:url}).catch(function(){}); }
          else if(navigator.clipboard){ navigator.clipboard.writeText(url).then(function(){alert('Link copied. You can repost it on your social media.');}); }
        }
        if(action==='share'){
          if(navigator.share){ navigator.share({title:document.title,text:'Discover this on AYMP',url:url}).catch(function(){}); }
          else if(navigator.clipboard){ navigator.clipboard.writeText(url).then(function(){alert('Link copied. Share it with your friends.');}); }
        }
        if(action==='copy'){
          if(navigator.clipboard){
            navigator.clipboard.writeText(url).then(function(){alert('AYMP link copied!');});
          } else {
            var area=document.createElement('textarea'); area.value=url; document.body.appendChild(area); area.select(); document.execCommand('copy'); area.remove(); alert('AYMP link copied!');
          }
        }
      });
    });
