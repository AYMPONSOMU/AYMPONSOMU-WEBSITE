...
(function(){
  ...
  function addMantraStyles(){
    ...
    s.textContent=`
      .aymp-mantra-glow-box{position:relative;margin:20px 0;padding:22px 16px;border:1px solid rgba(255,215,106,.58);border-radius:22px;background:radial-gradient(circle at 50% 15%,rgba(255,215,106,.16),rgba(52,25,82,.72) 42%,rgba(12,15,35,.94) 100%);box-shadow:0 0 18px rgba(255,215,106,.28),0 0 45px rgba(145,85,255,.20),inset 0 0 28px rgba(255,215,106,.06);overflow:hidden;animation:aympMantraGlow 3s ease-in-out infinite}
      .aymp-mantra-glow-box:before{content:'✦  ✧  ✦';position:absolute;top:6px;left:0;right:0;text-align:center;color:#ffd66a;letter-spacing:18px;opacity:.72;pointer-events:none;animation:aympMantraSpark 2.6s ease-in-out infinite}
      .aymp-mantra-glow-box:after{content:'';position:absolute;width:130px;height:130px;left:50%;top:50%;transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(255,215,106,.16),transparent 70%);pointer-events:none}
      .aymp-mantra-glow-box>*{position:relative;z-index:1}
      @keyframes aympMantraGlow{0%,100%{box-shadow:0 0 18px rgba(255,215,106,.22),0 0 38px rgba(145,85,255,.16),inset 0 0 24px rgba(255,215,106,.05)}50%{box-shadow:0 0 30px rgba(255,215,106,.42),0 0 62px rgba(145,85,255,.28),inset 0 0 34px rgba(255,215,106,.09)}}
  ...
  function boot(){
    ...
    restoreMantraBox();
    mountButton();
    const observer=new MutationObserver(function(){restoreMantraBox();mountButton();});
